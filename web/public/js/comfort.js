...
function renderSensorGroupGraph(sensorgroupid, targetdiv) {
    ...
    console.log('rendering graph for group id ' + sensorgroupid);
...
function populateSensors() {
    ...
    $(".nav-stacked > .sensor > a").on("click", function(event) {
        ...
        renderSensorGraph($(this)[0].id.replace("sensor ",""), "temperature", config.graphtime.value, $("#sensor-graph"));
...
function populateSensorGroups(activesensorid) {
    ...
    $(".nav-stacked > .sensorgroup > a").on("click", function(event) {
        ...
        if (controller != undefined) {
            console.log('labeling controller bar');
        ...
        $("#sensorgroup-controllerselect-list").append('<li><a id="controller 0" class="sensorgroup-controllerlist" href="#">none</a></li>');
        ...
        for(var key in sensors) {
            ...
            if (sensorgroups[sensorgroupid] != undefined && sensorgroups[sensorgroupid].members[key] != undefined) {
                li = '<li><label class="checkbox"><input id="sensorgroup-sensor ' + key +
        ...
        $('#sensorgroup-controllerselect-list > li > a').on('click', function(e) {
             assignControllerToSensorGroup(sensorgroupid, $(this)[0].id.replace('controller ',''));
        });
...
function addSensorToGroup(sensorid, sensorgroupid) {
    $.ajax({url: "/api/sensorgroups/" + sensorgroupid + "/sensors/" + sensorid,
            ...
            async: false,
            success: function() { 
                $("#sensorgroup-sensorselect-list").notify("added", "info", {autoHideDelay:1000}); 
...
function removeSensorFromGroup(sensorid, sensorgroupid) {
    $.ajax({url:     "/api/sensorgroups/" + sensorgroupid + "/sensors/" + sensorid,
            ...
            async:   false,
            success: function() {
                $("#sensorgroup-sensorselect-list").notify("removed", "info", {autoHideDelay:1000});
